...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Desventajas
		\begin{itemize}
		\item Presencia de ruido spekle
		\medskip
		\item Bajo contraste
		\medskip
...
\begin{frame}
	...
	\begin{itemize}
		...
		\medskip
		\item Los radares SAR emiten y reciben radiación horizontal o verticalmente polarizada.
		\medskip
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{itemize}
	\bigskip
	\item Uno de los objetivos centrales es la medición de la humedad del suelo. Por eso operan en banda L que tiene la capacidad de penetrar a través de la superficie hasta $\SI{2}{\meter}$ de profundidad dependiendo del tipo de suelo.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item \citet{Frery97,Frery99} presentan la familia de distribuciones $\mathcal{G}^0$ para datos dados tanto en formato amplitud como en intensidad, generando la familia de distribuciones $\mathcal{G}_A^0$ y $\mathcal{G}_I^0$ respectivamente.
		\bigskip
		...
		\end{itemize}
		\bigskip
			donde $-\alpha,\gamma ,z>0$ and $L\geq 1$ son el parámetro de textura, de brillo y el número de looks respectivamente.\\
...
\begin{frame}
	\frametitle{Densidad}
	La función de densidad del retorno $Z$ esta dada por:
	\begin{equation*}
...
\begin{frame}
	...
	\begin{block}{}
		Una de las características más importantes de la distribución $\mathcal{G}_I^0$ es la interpretación del parámetro $\alpha$  en términos de la textura de la zona observada.  
	\end{block}\pause
	\begin{table}
		\centering
		\begin{tabular}{|c|c|c|c|}
			...
			\hline
			Textura &Extremadamente texturada &  Texturada & Sin textura\\
			\hline
		\end{tabular}
	\end{table}\pause
	...
	\medskip
	\begin{itemize}
		\item En lo siguiente vamos a emplear el valor $\gamma^*$ such that $E(Z)=1$,  
		$$\gamma^* =-\alpha-1.$$ De esta manera hay solo un parámetro para estimar.
	\end{itemize}
	%\begin{block}{$\gamma^* =-\alpha-1$}
	%\end{block}
\end{frame}
...
\begin{frame}
	...
	Proponer un nuevo método de estimación para el parámetro de textura del modelo $\mathcal{G}_I^0(\alpha,\gamma,L)$, que tenga
	\bigskip
	\begin{itemize}
		\item  un buen comportamiento para muestras de tamaño pequeño y moderado.
		\medskip
		\item buenas propiedades midiéndolas en término de su sesgo, su error cuadrático medio, su capacidad para resistir diferentes niveles de contaminación.
		\medskip
...
\begin{frame}
	\frametitle{Máxima Verosimilitud}
	Si $Z_i \sim \mathcal{G}_I^0(\alpha,\gamma,L)$ asumiendo $\gamma=-\alpha-1$ con $L$ conocido,
	\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\bigskip
		\begin{itemize}
			\item Primer función característica \\de $2º$ tipo
			\begin{align*}
...
\begin{frame}
	\frametitle{Logcumulantes}
	\begin{itemize}
		\item Si $Z_1,\ldots,Z_n$ con $Z_i \sim \mathcal{G}_I^0(\alpha,-\alpha-1,L)$ es una muestra i.i.d., el estimador Logcumulantes del parámetro de textura $\alpha$, que lo llamaremos $\widehat\alpha_{\text{\tiny{LC}}}$, es la solución de la ecuación    
%		\begin{align*}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\bigskip
		\item En este sentido hay dos elementos involucrados: uno es la medida de distancia que permite cuantificar la proximidad, el otro es la información que proviene de la muestra. 
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Mínima Distancia}
	\begin{itemize}
		\item Teniendo en cuenta ambos elementos, si $Z_1,\ldots,Z_n$ es una muestra i.i.d. donde $Z_i \sim \mathcal{G}_I^0(\alpha,-\alpha-1,L)$  se define al MDE del parámetro $\alpha$ como
		\begin{align*}
		\widehat{\alpha}_n=\mathop{\rm argmin} \limits_{\alpha} d(f_{\alpha}, \widehat{f}_n),
		\end{align*}
		donde $d$ es una medida de distancia, divergencia o distancia estocástica, entre la función de densidad teórica $f_{\alpha}$ y un estimador no paramétrico $\widehat{f}_n$ de la función de densidad subyacente.
...
\begin{frame}
	...
	\bigskip
	\begin{itemize}
		\item  qué medida de distancia se va a utilizar.
		\bigskip
		\item Que estimador no paramétrico $\widehat{f}_n$ de la función de densidad subyacente.
	\end{itemize}
...
\begin{frame}
	...
	\begin{block}{}
		Cuando el soporte de la función de densidad a estimar no es acotado los núcleos simétricos tienen un buen desempeño y han sido muy estudiados.
	\end{block}{}
	\bigskip
	Pero\ldots
	\bigskip
	\begin{alertblock}{}
		Cuando el soporte de dicha función de densidad es acotado o semiacotado, los núcleos simétricos sufren de sesgo en el borde del soporte.
	\end{alertblock}{}
	\medskip
	\begin{alertblock}{}
...
\begin{frame}
	\frametitle{Núcleos Asimétricos}
	Una forma de resolver el problema es utilizar núcleos simétricos.
	\begin{definicion}
	\label{DefNucleo}
	Sea $Z_1, \ldots, Z_n$ una muestra aleatoria donde $Z_i \sim f$ con $f$ la función de densidad teórica, el estimador $\widehat{f}_n$ de $f$ utlizando núcleos asimétricos se define como 
...
\begin{frame}
	...
	\bigskip
	\item El Inverso Gaussiano (IG) y Recíproco Inverso Gaussiano (RIG) propuesto por~\citet{Scaillet2004} .
	\bigskip
...
\begin{frame}
	\frametitle{Alguno de los núcleos estudiados}
	\begin{itemize}
		\item Todos estos núcleos tienen soporte en $[0,+\infty)$, no asignan peso fuera del soporte de la distribución.
		\medskip
		\item \citet{Scaillet2004} indica que el sesgo en el borde de los núcleos $\Gamma$, IG y RIG el sesgo en el borde es un $O(b)$.
		\medskip
		\item \citet{Libnegue2013} indica que el sesgo correspondiente al núcleo LN es un $O(b^2)$.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	Estimar el parámetro de textura $\alpha$ como
	\begin{align*}
	\label{PropuestaMDE}
	\widehat{\alpha}=\mathop{\rm argmin} \limits_{\alpha} d(f_{\alpha}, \widehat{f}_n)
	\end{align*}
	donde $f_{\alpha}$ es la función de densidad $\mathcal{G}_I^0(\alpha,-\alpha-1,L) $, $d$ es una distancia estocástica y $\widehat{f}_n$ es un estimador no paramétrico de $f_{\alpha}$ utilizando núcleos asimétricos.
...
\begin{frame}
	...
	\bigskip
	Se realizaron $1000$ replicaciones generando muestras $Z_1, Z_2,\ldots,Z_n$ de variables aleatorias i.i.d donde $Z_i \sim f_{\mathcal{G}_I^0(\alpha,\gamma^*,L)}$ donde el espacio paramétrico está formado por:
	\vspace{2mm}
	\begin{itemize}
...
\begin{frame}
	...
	\bigskip
	\begin{itemize}
		\item se estima $\widehat{f}$ utilizando histogramas.
		\medskip
		\item se utiliza el método de Freedman Diaconis para elegir el ancho de banda $b$ dado por $b=2 \ \text{IQR}(z_1,\ldots,z_n) n^{-1/3}$ donde $\text{IQR}$ es el rango intercuartil.
		\medskip
		\item se calcula $\widehat{\alpha}= \mathop{\text{argmin}}\limits_{-10 \leq \alpha <-1}d(f_{\mathcal{G}_I^0(\alpha,-\alpha-1,L)},\widehat{f})$ donde $d$ es alguna de las distancias estocásticas consideradas.
		\medskip
...
\begin{frame}
	\frametitle{Diseñando el experimento}
	De esta manera se obtuvieron $1000$ valores estimados de $\widehat{\alpha}$. Con estos valores se estimó la esperanza, el sesgo y el error cuadrático medio que están definidos por:
	\bigskip
	\begin{itemize}
...
\begin{frame}
	...
	\begin{figure}[htb]
		\setcounter{subfigure}{0}
		\centering    
		\subfigure[\small $\widehat{\text{Sesgo}}$]{\includegraphics[width=.40\linewidth]{../../Figures/Tesis/Capitulo6/SesgoASTBarrasError_L3ypercentil.pdf}}
		\subfigure[\small $\widehat{\text{ECM}}$]{\includegraphics[width=.40\linewidth]{../../Figures/Tesis/Capitulo6/ECMASTBarrasError_L3ypercentil.pdf}}
	\end{figure}	
\end{frame}
...
\begin{frame}
 ...
 Presentamos mejoras ya que: 
	...
	\begin{itemize}
		...
		\medskip
		\item Se consideró $\alpha \in [-20,-1)$ pues...
	\end{itemize}
...
\begin{frame}
		...
		\begin{itemize}
			...
			\medskip
			\item El primer tipo de contaminación asume que, con probabilidad $\epsilon$, los datos pueden provenir de una distribución perteneciente a la familia de distribuciones $\mathcal{G}_I^0$ pero con otros parámetros.
			\medskip
			\item Una de las fuentes de contaminación en las imágenes SAR es el fenómeno de \textit{double bounce}, donde algunos píxeles tienen un alto valor de retorno. Por eso estudiamos el caso donde se contamina, con probabilidad $\epsilon$, con una constante con valor grande respecto de la media.
		\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item Imagen E-SAR con $L=1$, banda \textit{L} polarización HH datos de intensidad.
		format E-SAR
	\end{itemize}
	\begin{figure}[htb]
		\centering
		...
		%\caption{\label{MuestrasCorner} Muestras de varios tamaños en una .}
	\end{figure}
\end{frame}
...
%		\midrule
		\multirow{5 }{*}{$-3$}	
		&   9    	&  	  0.253  	&	  0.563  	&	  12.473  	&	  50.020  	\\
		&   25   	&  	  0.080  	&	  0.106  	&	  3.217   	&	  25.401  	\\
		&   49   	&  	  0.044  	&	  0.071  	&	  0.686   	&	  16.401  	\\
		&   81   	&  	  0.030  	&	  0.034  	&	  0.255   	&	  11.577  	\\
		&   121  	&  	  0.022  	&	  0.029  	&	  0.166   	&	  8.985  	\\
		\midrule
%		\multirow{5 }{*}{$-5$}	
...
%		\midrule
		\multirow{5 }{*}{$-8$}	
		&   9    	&  	  0.237    	&	  0.383  	&	  18.106  	&	  73.081  	\\
		&   25   	&  	  0.072  	&	  0.112  	&	  5.134   	&	  40.479  	\\
		&   49   	&  	  0.041  	&	  0.048  	&	  0.930   	&	  30.231  	\\
		&   81   	&  	  0.026  	&	  0.031  	&	  0.329   	&	  25.712  	\\
		&   121  	&  	  0.017  	&	  0.020  	&	  0.214   	&	  21.193  	\\
		%\bottomrule
	\end{tabular}																		
\end{table}	
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\bigskip
		\item En términos de sesgo, ECM y cantidad de casos de no convergencia.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{minipage}{0.45\textwidth}
		\begin{figure}
			\subfigure{\includegraphics[width=.68\linewidth]{../../Figures/Tesis/Capitulo6/GraficoSesgoMVyGAyLNyLC_L=3Cont_BarrasErroryPercentil.pdf}}\setcounter{subfigure}{0}
	...
	\begin{minipage}{0.45\textwidth}
		\begin{figure}
			\subfigure{\includegraphics[width=.68\linewidth]{../../Figures/Tesis/Capitulo6/GraficoECMMVyGAyLNyLC_L=3Cont_BarrasErroryPercentil.pdf}}\setcounter{subfigure}{1}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{table}
			\begin{tabular}{S[table-format=7.2] S[table-format=7.2] S[table-format=7.2] S[table-format=7.2]}
				...
				1313430.89 &	 \color{red} \textbf{76311.14}  &	212135.11 &	404060.49\\
				\bottomrule
			\end{tabular}
		\end{table}
	\vspace{0.8cm}
	\item Estimaciones sin la observación más baja.
		...
		\begin{table}
		\begin{tabular}{cccc}
			$\widehat\alpha_{\text{\tiny{MV}}}$ & $\widehat\alpha_{\text{\tiny{G}}}$ &$\widehat\alpha_{\text{\tiny{LN}}}$ & $\widehat\alpha_{\text{\tiny{LC}}}$\\
			\midrule
			-5.73  &	-3.97	&	-3.13 &	-4.80
		\end{tabular}
	\end{table}
	\vspace{0.8cm}
	\item Todas las estimaciones corresponden a una zona heterogénea.
	\end{itemize}
...
\begin{frame}
	...
	\begin{Teorema}
			...
			Sea $\widehat{f}_{n}$ el estimador de $f$ utilizando núcleos asimétricos y que verifica las condiciones anteriores. Supongamos que existe un número real $r_1=r_1(K)>0$ que depende del núcleo elegido tal que, para cada $x \in \text{Sop}(f)$ 
			\begin{align*}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{align*}
		donde $k_n$ es una sucesión que tiende a cero cuando $n$ tiende a infinito, entonces. 
...
\begin{frame}
	...
	\begin{Teorema}
		\medskip
		\small
		Sea $Z_1, \ldots, Z_n$ una sucesión de variables aleatorias iid donde $Z_i \sim f_{\alpha_{*}}$ con $f_{\alpha_{*}} \sim \mathcal{G}_I^0(\alpha^*,-\alpha^*-1,L)$  y sea $\widehat{f}_n$ un estimador por núcleos asimétricos de la función de densidad subyacente que cumple las condiciones del teorema anterior. Sea $\{\widehat{\alpha}_n\}$ una sucesión MDE estimadores como los definidos en~\eqref{EstMinDistRelajado}.
...
\begin{frame}
	...
	\begin{itemize}
		...
		\medskip 
		\item Está basado en la minimización de una distancia estocástica entre el modelo y una estimación de la función de densidad subyacente $\widehat{f}$ utilizando núcleos asimétricos. 
		\medskip
