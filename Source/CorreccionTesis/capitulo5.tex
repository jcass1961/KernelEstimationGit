...
\begin{itemize}
	...
	\begin{align}
	\label{MISEoptGamma}
	%\nonumber MISE(\widehat{f}_{\Gamma})=&b^2 \int_0^{\infty} (x f'(x)+\frac{1}{2} x f''(x))^2 dx + \frac{1}{2 \sqrt{\pi}} \frac{1}{n \sqrt{b}} \int_0^{\infty} x^{-1/2} f(x) dx \\
...
\begin{align}
\label{ISE}
ISE(b)=&\int_\mathbb{R} (\widehat{f}_b-f)^2 dx \nonumber\\ 
...
\begin{align}
\nonumber \min_b ISE(b)= \min_b \phi(\widehat{f}_b) \text{ donde } ,
\end{align}
donde
\begin{align}
\label{fi}
