...
\begin{align}
\label{PropuestaMDE}
\widehat{\alpha}=\mathop{\rm argmin} \limits_{\alpha<0} d(f_{\alpha}, \widehat{f}_n)
\end{align}
...
\begin{itemize}
	...
	\item se calcula $\widehat{\alpha}= \mathop{\text{argmin}}\limits_{\alpha<0}d(f_{\mathcal{G}_I^0(\alpha,-\alpha-1,L)},\widehat{f})$ donde $d$ es alguna de las distancias estocásticas definidas en la sección~\ref{dist}.
\end{itemize} 
...
\end{align}
donde $\widehat{f}_{b,\text{K}}^i(x)$ es el estimador de la función de densidad subyacente en la replicación $i$ para el núcleo $\text{K}$, que depende del 
%%% ACF ¿Por qué texto, si está en modo ecuación en la definición?
ancho de banda $\text{b}$.
