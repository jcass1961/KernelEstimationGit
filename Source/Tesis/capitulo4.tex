...
\begin{itemize}
	\item Un enfoque paramétrico: donde se considera que la función de densidad teórica pertenece a una familia de funciones de densidad $f_X(\vec{\theta})$\todo{¿Vas a usar la notación vectorial explícita? Se me hace un poco pesada.}
...
Si en cambio $X_i \sim f_{X;\vec{\theta}}$ con $\theta \in \Theta \subset \mathbb{R}^k$,\todo{Cuidado con la notación de $\theta$.} entonces $\hat{\theta}_{\text{mom}}$ será el valor $\vec{\theta}$ que verifica el sistema de ecuaciones $\dfrac{1}{n} \sum_{i=1}^n g_s(X_i)=E_{\theta}(g_s(X_i))$ donde $s=1, \ldots k$ y $g_s(x)$ son $k$ funciones de $\mathbb{R}$ en $\mathbb{R}$.
\end{definition}
...
Las ecuaciones de momento de orden $1$ y de orden $\frac{1}{2}$ para estimar los parámetros de la distribución $\mathcal G_I^0$ son:\todo{Todavía no presentaste la distribución, y no definiste $\overline X$.}
\begin{align}
...
\subsection{Método de Máxima Verosimilitud}
 ...
 El Método de Máxima Verosimilitud fue popularizado por R. Fisher entre 1912 y 1922. Sin embargo este método había sido utilizado antes por Gauss, Laplace entre otros investigadores. La idea del método consiste en, dada una muestra, estimar los parámetros de la distribución que maximicen la probabilidad\todo{¿Pro\-ba\-bi\-li\-dad o verosimilitud?} de que haya salido esa muestra. Este estimador tiene muy buenas propiedades asintóticas, además de ser consistente es asintóticamente normal y eficiente bajo condiciones generales.
...
\end{align}
\todo{Revisá toda la puntuación después de ecuaciones centralizadas. O terminan con punto, o terminan en coma.}
...
%%% ACF No dejes espacio antes de las ecuaciones
\begin{align}
...
\end{align}
\todo{Revisá la ecuación, sacá los paréntesis que no son imprescindibles, usá mid para "dado", etc.}
...
%%% ACF Como después te referís al número de la propiedad, deberían estar enumeradas
\begin{itemize}
	\label{metrica}
...
\begin{definition}[$\phi \text{-} divergencias$]
	...
	\end{align}\todo{¿Tiene sentido poner $dx$?}
...
\begin{example} Ejemplos de $\left(h,\phi\right) \text{-} divergencia$ son:
	\begin{itemize}
		...
		\end{align}
	\end{itemize}
\todo{Si las $\phi$ son casos particulares de las $h$-$\phi$, basta decirlo y agregar acá alguna que no sea $\phi$.}
\end{example}
...
Sea $d_{s}\colon\R_{+}\times\R_{+}\to\R_{+}$ la función definida por
\begin{align*}
d_{s}\pa{a,b} = 
\begin{cases}
...
\todo{\dots\ pero que en algunas situaciones se vuelve plana y, por lo tanto, de difícil minimización.}
...
\label{fn}
\end{equation}
donde  $b$ es el ancho de banda, ${\theta}(x,b)$ es el vector de parámetros\todo{¡Ojo con la notación!} que depende del núcleo usado, y $K$ el núcleo asimétrico.
\end{definition}
...
VER ABAJO COMENTADO\todo{Dejar claro qué viene de la literatura, y qué es tuyo, aunque sean ajustes de notación.}
