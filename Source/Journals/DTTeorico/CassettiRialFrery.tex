...
\begin{abstract}
	The $\mathcal{G}^0$ family of distributions has been widely used in recent years to analyze and interpret data from sensors with coherent illumination automatically. Special attention has been devoted to modeling data that come from images of Radar Synthetic Aperture using a multiplicative model and the $\mathcal{G}^0$ distributions for intensity data. This distribution has support positive and is indexed by three parameters: texture, scale and number of looks. In this paper the consistency of the minimum distance estimator for the texture parameter is proved, for the multilook case and the $\mathcal G_I^0$ model. Triangular distance is employed and the estimator is defined as the argument that minimized this stochastic distance between the theoretical distribution and a nonparametric estimation of the underlying density function based on asymmetric kernel. 
...
\label{intro}
\PARstart{S}{ynthetic} Aperture Radar (SAR) imagery are an important tool used in
...
\begin{proposition}
	...
	\begin{enumerate}[i.]
		...
		\item\label{it: unicidad} $d_{T}\pa{f;g} \ge 0$ y $d_{T}\pa{f;g} = 0$ if and only if $f = g$.
...
\begin{dem}
	...
	The inequality~\eqref{it: acotacion} is a consequence of the following inequality $\abs{\frac{\pa{a-b}^{2}}{a+b}} \le \abs{a-b}$ taking $a = f\pa{x}$ y $b = g\pa{x}$ and integrating.
	% \begin{align*}
...
\begin{IEEEbiography}[{\includegraphics[height=1.7in,width=1.1in]{../../../Images/Fotos/JuliaGris7}}]{Julia Cassetti} 
