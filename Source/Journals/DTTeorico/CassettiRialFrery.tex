...
\textcolor{blue}{Different distributions have been proposed for modelling SAR images. Log-Normal and Weibull were introduced to caractherize high resolution data~\cite{oliverquegan98}, \textit{K} and Weibull were considered in~\cite{Oliver1993}, in~\cite{Tison2004} the authors present  Fisher distribution to model several kinds of areas. In~\cite{Li2011} the authors proposed a Generalized Gamma distribution for modeling SAR images where, for different values of the parameters, it can be obtained Weibull, \textit{K} Fisher distributions between others.}

\textcolor{blue}{An alternative approach is the multiplicative model, which have been successfully used to described SAR data. It is based on the assumption that the return results from the product between two random variables: one corresponding to the backscatter (the unobserved truth) and the other to the speckle~\cite{oliverquegan98}. }
%Other models have been used; Ref.~\cite{Gao2010} discusses several approaches and their relationships.

%%% ACF Hasta acá 1/6/18 13:40
...
\textcolor{blue}{In recently years the $\mathcal G$ model~\cite{Frery97} has been widely utilized to depict the return of SAR images under the multiplicative model. It has the ability to discriminate textured and extremely textured areas better than other models~\cite{MejailJacoboFreryBustos:IJRS}. It is characterized by three parameters, the $\alpha$ parameter that allows to explain different areas in terms of their texture, the $\gamma$ parameter informs about the brightness of the image and the $L$ parameter is related to the signal-to-noise ratio. 
Therefore the accuracy in the parameters estimation is a crucial problem, in particular the $\alpha$ parameter, because it describes locally the target roughness.}
...
\textcolor{blue}{Minimum Distance Estimators (MDEs) are another alternative to avenue the problem of parameter estimation of a distribution with good properties.
...
Images generated with devices that employ coherent illumination are contaminated with speckle. 
This kind of noise is neither Gaussian nor additive. 
In the monopolarized case and intensity data, SAR images can be modeled as the product of two independent random variables, one corresponding to the backscatter $X$ and other to the speckle noise $Y$. 
In this manner $Z=X Y  $ represents the return $Z$ in each pixel.
%The multiplicative model with the $\mathcal{G}_I^{0}$ model is a good combination to successfully describe areas with different degree of texture~\cite{Frery97,MejailJacoboFreryBustos:IJRS}. 
