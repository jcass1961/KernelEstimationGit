...
\textcolor{blue}{The classical kernel estimator, which uses symmetric kernels, is extensively used to estimate the underlying density function. But when this function has bounded support, this estimator presents boundary bias. This bias appears because the symmetric kernel estimator assigns weight outside the support when the estimation take place near the boundary.}
...
It is important to note that these kernel estimators are free of boundary bias and achieve the convergence rate $n^{-4/5}$ for the MISE.
 ...
 $\gamma_0$ is chosen such that the expected value of the return $Z$ is one. This condition leads to the following relation: $\gamma_0^*=-\alpha-1$.

\subsection{Logcumulant estimator}\label{lc}

%%% ACF This paragraph is not clear
\textcolor{blue}{It is known the relationship between the random variable moments and its characteristic function, defines as the Fourier transform of its probability density function (pdf). 
...
The presence of such outliers may provoke big errors in the estimation as we show in an application to a real image. \textcolor{blue}{This situation can be modeled considering the return $Z=BC+(1-B)W$ where $C$ is a large value of the return and $B$ and $W$ are defined as before.}
...
%%% ACF "exceeds"? 
areas in terms of bias, while $\widehat{\alpha}_{\text{{LN}}}$ \textcolor{blue}{outperform} the rest in extremely textured areas and with moderate texture.
...
It can be seen that both $\widehat{\alpha}_{\text{{ML}}}$ and $\widehat{\alpha}_{\text{{LC}}}$ have substantially changed the estimations value \textcolor{blue}{between the yellow and the red samples, indicating different kind of textures.} While $\widehat{\alpha}_{\Gamma}$ and $\widehat{\alpha}_{\text{{LN}}}$ changed their values but the type of zone they describe was not modified.
