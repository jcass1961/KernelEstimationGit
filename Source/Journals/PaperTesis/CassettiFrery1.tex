...
\title{Improvments to the Minimum Distance Estimator for the Texture Parameter of the $\mathcal{G}_I^0$ model}
\author{ Julia~Cassetti and Alejandro~C.~Frery,~\IEEEmembership{Senior Member}

\thanks{This work was supported by CNPq, and Fapeal.}
...
This paper proposes an improvement to the minimum distance estimator to the texture parameter $\mathcal{G}_I^0$ law, using the triangular distance between $\mathcal G_I^0$ density and an estimate of the underlying density function using Inverse Gaussian kernel (IG). The proposal consists of using Gamma and Lognormal kernel together with the Least Square Cross Validation Method to find the appropriate bandwidth. This kernels outperform the IG in terms of integrated mean square error and number of convergence cases.
...
We apply these estimators to actual images, with the purpose of identifying areas with different textures, using windows of several sizes to compare their performance. The experimental results demonstrate the effectiveness of the proposed method.
\end{abstract}

\begin{keywords}
Feature extraction, texture analysis, kernel estimation, synthetic aperture radar, parameter estimation, minimun distance estimator.
\end{keywords}
...
The multiplicative model has been extensively used in the modeling and analysis this kind of images. 
...
The texture is of paramount importance, as it describes locally the target roughness.
...
Refs.~\cite{VasconcellosFrerySilva:CompStat,CribariFrerySilva:CSDA} describe attempts to improve the performance of this estimators with respect to bias and mean squared error using either resampling or analytic procedures. 
More recently, using a connection between the Mellin's transform, a number of estimators based on logcumulants and logmoments have been proposed~\cite{MellinAnalysisPolSAR,BujorTrouveValetNicolas2004,khan2014}. 
...
It is desirable that a good estimator has a good behavior if the data comes from the assumed model and that do not present large deviations against small perturbation from the assumed model.
...
Some authors aim to use asymmetric kernels estimators to solve this problem. Chen~\cite{chen1999,chensx2000} presents Beta and Gamma ($\Gamma$) kernels,~\cite{Scaillet2004} introduce inverse Gaussian (IG) and reciprocal inverse Gaussian (RIG) kernels,~\cite{bouezmarni2005} prove consistency of Gamma, IG an RIG kernels. Refs.~\cite{Jin2003} proposed Birnbaum-Saunders (BS) and Lognormal (LN) kernels. The kind of kernels is trated also in~\cite{libengue2013}. These estimators vary their shape according to the observation, a feature that allows obtaining different degrees of smoothing without incurring in the aforementioned problems~\cite{Scaillet2004}.

In the last decade, concepts related to information theory have gained interest and application to image processing. In particular, several measures have been proposed to reflect the closeness between two distribution function. Specially the concept of stochastic divergence has found applications in areas as diverse as signal and image processing~\cite{Aviyente2007}, automatic region
detection in SAR imagery~\cite{SilvaCribariFrery:ImprovedLikelihood:Environmetrics,Nascimento2009}, applying filters to reduce speckle noise~\cite{Penna2019}, as contrast measures. Unsupervised classification strategies were applied to Polarimetric Synthetic Aperture Radar (PolSAR) imagesn in~\cite{Carvalho2019}.

In~\cite{APSAR2013ParameterEstimationStochasticDistances} the authors proposed a MDE estimator to the texture parameter of the $\mathcal{G}^0$ model for intensity data and assessed differents stochastics distance with a non-parametric estimation of the underlying density function using histograms. In Gambini et al.~\cite{gambini2015} the authors proposed to estimate $f$ using the IG asymmetric kernel with a bandwidth chosen empirically, and concluded that the triangular distance is the best suited for this problem.  They show that their proposal outperforms the Logcumulant estimator in terms of mean squared error, bias, convergence and robustness. So, the results were promising, with venues for improvement, some of which are explored in this work.
...
In this paper, we introduce enhances to the Gambini et al.~\cite{gambini2015} proposal. We use the $\Gamma$ and LN kernels instead of IG kernel, showing that the latter presents a larger integrated mean square error  and higher percentage of non-convergence cases than the others.
Regarding the choice of bandwidth, we use Least Squared Cross-Validation (LSCV)~\cite{Rudemo1982} for bandwidth selection in place of the empirical selection of~\cite{gambini2015}. Further we use an algorithm to find the minimum of the MDE estimator in lieu of range a grid of texture parameter values. 

We compare the performance of the proponed estimator for the texture parameter of the $\mathcal{G}^0$ model for intensity data with Maximum Likelihood and LogCumulants methods in terms of convergence, bias and mean squared error, and sensitivity to contamination. We obtain good results specially for small samples size.

%We compare the performance \textcolor{blue}{of the proposed estimator with the one proponed in~\ref{gambini2015}} several estimation procedures based on nonparametric (asymmetric kernels) and parametric (tail index estimators) 
...
%Section~\ref{pareto} shows the relation between this model and the Pareto distribution, and proposes an new estimator for the single-look case.
...
This gives rise to the $\mathcal{G}_I^{0}$ distribution for the return $Z$.
Given the mathematical tractability and descriptive power of the $\mathcal{G}^{0}$ model~\cite{MejailJacoboFreryBustos:IJRS,mejailfreryjacobobustos2001} it represents an attractive choice for SAR data modeling.
...
It is important to achieve accurate and robust estimates of $\alpha$ because  it is related to the texture of the target. 
...
With the purpose of reducing our analysis to a single parameter, we based the forthcoming analysis on the condition $E(Z)=1$ which links the texture and the brightness parameter $\gamma^* =-\alpha-1$.
...
We employed the L-BFGS-B version of the Broyden-Fletcher-Goldfarb-Shanno (BFGS) method~\cite{Luenberger2008} to find the optimum of a function. This algorithm is a quasi-Newton type optimization method that allows box constraints, and is widely used in graphic computing and scientific computing~\cite{Fei2014}. The characteristic of these methods is that they do not require to calculate the Hessian matrix but uses an approximation of it that is updated in each iteration, using only the function and its gradient.
...
Minimum distance estimators (MDE) is a non-parametric methodology that propose new estimators of the parameters of a distribution. Its purpose is to find the value of the parameters that make the theoretical model as close as possible to the information provided by the sample. In this sense there are two elements involved: one is the measure of distance that allows quantifying proximity, the other is the information that comes from the sample.
If we rely on the sample information, there are measures that link the empirical distribution function and the theoretical model, and others that measure the discrepancy between a non-parametric estimate of the underlying density function and the truth density. Because, in general, the distribution function is absolutely continuous, it can think of the MDE estimators in terms of the density function. Thinking
...
\end{align}
where $d$ is a dissimilarity  measure or stochastic distance between the theoretic density function $f_{\mathcal{G}_I^0}$ and $\widehat{f}_n$ a non-parametric estimator of the underlying density function.
...
In Gambini et al.~\cite{gambini2015} the underlying density function was estimated using asymmetric kernels, in particular, the authors used the IG kernel which is defined in subsection~\ref{asymmetrickernel}. 
In this work we use $\Gamma$ an LN kernels because we show that they outperform IG kernel since they have less integrated mean quadratic error and fewer cases of non-convergence than IG kernel.
...
It is important to note that these kernels estimators are free of boundary bias and achieve the convergence rate $n^{-4/5}$ for the MISE.
...
It should be noted that one of the points of interest in this work is to find a texture parameter estimator of the $\mathcal{G}_I^0$ distribution which has a good behavior for small sample sizes. This aspect is interesting of
keep in mind because a lot of the methods of image filtering or edge detection used sliding masks to estimate parameters. These mask are usually be of size $3 \times 3$, $5 \times 5$, $7 \times 7$, $9 \times 9$ and $11 \times 11$. In this sense it is known that the MLE estimator cannot present good behavior when sample size
is small. For this reason it is necessary to find estimators that
have asymptotic behavior similar to MLE and outperform the latter for small sample sizes and for deviations from the theoretical model.
...
\label{lc}
It is known the relationship between the moments of a random variable and its characteristic function, defines as the Fourier transform of its probability density function (pdf). Using this approach moment estimators are defined. In the same sense, Nicolas et al.~\cite{nicolas2002} defined the first characteristic function of the second kind as the Mellin transform of its pdf function. From this, if $X \sim p_x(u)$ it can be defined:
\begin{itemize}
...
This estimator is widely used in in Sar image processing. Refs~\cite{MellinAnalysisPolSAR,BujorTrouveValetNicolas2004,khan2014} applied this methodology due to its nice properties and good performance. More recently, Nogeira et al.~\cite{Nogueira2019} used Logcumulants method to estimate the parameters of the $\mathcal{G}_I^0$ model to apply an unsupervised algorithm for SAR image segmentation.

Because both Logcumulants and MLE estimators are used in the literature, we compare the performance of our proposal with these methods.
...
We considered the Bernoulli random variable $B$ with probability of success $\epsilon$ that indicates the contamination ratio.  
This schema considers that in a very small proportion $\epsilon$ the data comes from the same model but with different parameters.  
...
Another way of assessing how the estimator $T_n(z_1,\dots,z_n)$ behaves under contamination is fixing $n-1$ observations and allowing one to vary.
...
Four estimators are compared for the multilook case: 
...
The confidence interval of approximately $95\% $ level is also plotted for each of the estimates made. The percentile method described in~\cite{Buckland1983} was used. This method consists in using the percentile $(\alpha/2)$ and $(1-\alpha/2) $ of the distribution of $\widehat{\alpha}$. This method was evaluated by~\cite{Buckland1983} in the case where the underlying distribution is an exponential where the symmetry of the distribution fails. The author shows that this interval has a better performance than the confidence interval using the normal approximation method. 
...
\begin{figure}[htb]
	...
	\caption{\label{SesgoyECMSinContL=3}\small Sesgo y MSE estimados para datos sin contaminar y $L=3$.}
\end{figure}	
...
The horizontal line is the true.
...
%
In the following we show the performance, throughout the bias and mean square error, of the estimators under contamination data. Figures~\ref{SesgoyECMConContL=3} and~\ref{SesgoyECMConContL=8} present contaminated data for $L=3$ and $L=8$ respectively. In this case we consider $\epsilon=0.05$
...
We have applied these methods to a real image of one look from around Munich taken by an E-SAR~\cite{Horn1996} L band, HH polarization in intensity format. This image has $300\times250$ pixels and mainly comprises two different growing areas. The original image is single-look, in order to obtain comparable multilook data we computed new pixels averaging the intensity values in $2\times2$ non-overlapping windows. This technique is known as pyramid processing~\cite{Adelson1984}, the resulting image is a ``multilook'' one.
...
In figure~\ref{TresMuestras} three samples are plotted: yellow, red and blue. The sample size of the first two are $16$, the latter has sample size $12$. The red sample is a shift from the yellow one in a row of the image data matrix, the blue sample is contained within the yellow sample.

Table~\ref{TablaTresMuestras} shows the estimates values obtained with the different methods in each of the samples. It can be seen that, in the yellow sample, the estimations for the different methods differ considerably. Both $\widehat{\alpha}_{\text{MV}}$ and $\widehat{\alpha}_{\text{LC}}$ give estimates of texture parameter compatible with a homogeneous zone, while $\widehat{\alpha}_{\Gamma}$ indicates that it is an heterogeneous one. Also $\widehat{\alpha}_{\text{LN}}$ is on the limit of affirming whether the area is heterogeneous or extremely heterogeneous.
...
In this new analysis, the behavior of the estimators under study is revealed. It can be seen that $\widehat{\alpha}_{\text{MV}}$ and $\widehat{\alpha}_{\text{LC}}$ have a similar performance, and on the other hand this is the same manifest with $\widehat{\alpha}_{\text{LN}}$ and $\widehat{\alpha}_{\Gamma}$. In addition, for $n=$ 9, none of the MV and LC methods converge. As the sample size increases, the estimates stabilize showing that the LN method is the most stable while $\widehat{\alpha}_{\text{MV}}$ and $\widehat{\alpha}_{\text{LC}}$ are the most unstable, giving poor estimates for small sample sizes.

It can also be seen that the confidence intervals corresponding to the MV and LC methods are generally wider than those of the other methods. All intervals decrease in length as the sample size increases, that is, they become more precise. It is important to mention that the confidence interval corresponding to the LN method is the one with the shortest length in most cases. It should be noted that, in the case where the MV and LC methods did not converge, there is no confidence interval and only the samples where all the methods converged were considered.


\section{Conclusions}
\label{conclusion}

This work presents improvements to the MDE estimator for the texture parameter of the $\mathcal{G}^0$ model for intensity and multilook data, with respect to the studied en~\cite{gambini2015}. We assess two other kernels: Lognormal and $\Gamma$, we use the LSCV method to obtain the bandwidth instead of use a fixed one empirically chosen, and we also implemented an optimization procedure to find the minimum, maximum and the roots of the functions associated with each of the estimators analyzed. 
...
We show that the IG kernel together with fixed bandwidth as the one obtained by the LSCV, have higher MISE in all of the studied cases, and higher percentage of non-convergence cases in most of the instances analyzed with respect to the one presented by the $\Gamma$ and Lognormal kernels. For these reason we choose the latter to continuous the analysis.

In the Monte Carlo study it can be seen that: a) On the one hand, the MV and LC methods have similar behavior to each other and, on the other, the MDE estimators using LN and $\Gamma$ kernels, for both uncontaminated and contaminated data. Even this behavior occurs when analyzing the SEIF function, b) For uncontaminated and textured data, the MDE estimator is competitive with respect to the MV and LC ones in terms of bias and MSE. However, it outperforms the MV and LC estimators in the presence of contamination, either using $\Gamma $ or LN kernels, although the latter is the one which presents the most stable behavior, c) The percentage of non-convergence cases presented by MDE estimators is significantly lower compared to the MV and LC estimators and d) The computational time of the MDE estimators is significantly greater with respect to the MV and LC estimators.

In the application to a real image MDE together with Lognormal kernel showed the best performance, exceeding MV and LC estimators. Based on the foregoing, the MDE estimator has good properties measured by its bias, mean square error and number of cases of non-convergence. It is competitive with the MV estimators and LC estimators in situations without contamination, and exceeds these methods in the presence of small levels of contamination.

For this reason our conclusion is that it is advisable to use $\widehat{\alpha}_{\text{\tiny{T}}}$ with LN kernel, especially when using small samples and / or against the presence of contaminated data. Although the additional computational cost of the MDE estimator is much greater than that required to calculate $\widehat{\alpha}_{\text{\tiny{ML}}} $, its advantages are more important than this increase in processing time
